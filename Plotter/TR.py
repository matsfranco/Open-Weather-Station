...
matplotlib.rcParams['axes.color_cycle'] = ['r','c','b','k']
...
sql = "select time,temp,rain from Log"
cursor.execute(sql)
...
plt.grid(True)
plt.ylabel('Temp C, Rain')
plt.axis(ymax=100,ymin=0)
plt.xlabel(time.asctime())
plt.title("<Floradas> Temperature (Red), Rain (Ciano)")
plt.hold(True)
...
FileName = '/home/pi/WeatherStation/Python/Graphics/TR.jpg'
plt.savefig(FileName)

print 'Copy to Web Server...',
Destination = '/var/www/images/'
os.system("sudo cp //home/pi/WeatherStation/Python/Graphics/TR.jpg {}".format(Destination))
